...
public class WriteExcel {
  ...
  public static void main(String[] args) {
    ...
    for(int i=0; i<10; i++){
      list.add(new String[]{"是打发斯蒂芬","13245678988","是是是","北京","北京","东城区","东华门街道","是可怜的房价快速的了解分开始了"});
    }
    WriteExcel deh=new WriteExcel();
    deh.setTitle("log日志");//添加报表标题

    String[] title={"供应商名称","联系电话","联系人", "所在地区-省", "市", "区", "街道", "详细地址"};
    deh.setHeader(title);
    //添加报表的查询字段名称
    ...
    boolean bool=deh.write(list,"/Users/linrufeng/workOrder.xls");//第一个参数：数据集合；第二个参数：存储路径（我这里是本地路径），如果是在servlet中则是：request.getSession().getServletContext().getRealPath("/")+"项目中的文件夹名字/workOrder.xls";
